 ...
 *    searchBaseGroups {String}
 *        The base DN from which to search for groups by username.
 *         E.g. 'ou=internal,o=example.com'
 *    searchScope {String}
 ...
 *        LDAP search filter with which to find a group by username, e.g.
 *        '(&(uid={{username}})(objectclass=person)(ismemberof=*))'. Use the literal '{{username}}' to have the
 *        given username be interpolated in for the LDAP search.
 *    searchAttributes {Array}
 *        Optional, default all. Array of attributes to fetch from LDAP server.
 *    searchGroupAttributes {Array}
 *        Optional, default 'ismemberof'. Array of attributes to fetch from LDAP server.
 *    log4js {Module}
...
function LdapAuth(opts) {
  ...
  assert.ok(opts.searchBaseGroups);
  assert.ok(opts.searchFilterGroups);
    ...
    self._adminClient.search(self.opts.searchBase, opts, function (err, result) {
      ...
      result.on('end', function (result) {
        ...
        case 0: // NO USER FOUND
          return callback();
        case 1: // FOUND THE USER
          return callback(null, items[0]);
        default: // MULTIPLE USERS FOUND
...
/**
 * Find the groups for the given username.
 *
 * @param username {String} e.g., "ritsid=1000063717,ou=internal,o=citigroup,c=us"
 * @param callback {Function} `function (err, groups)`. If no such groups is
 *    found but no error processing, then `groups` is undefined.
 *  - 'ismemberof' is an optional attribute, so it needs to be specified/requested
 */
LdapAuth.prototype._findGroups = function (username, callback) {
  ...
  self._adminBind(function (err) {
    ...
    var searchFilterGroups = self.opts.searchFilterGroups.replace(/{{username}}/g, username);
    var opts = {filter: searchFilterGroups, scope: self.opts.searchScope};
    if (self.opts.searchGroupAttributes) {
      opts.attributes = self.opts.searchGroupAttributes;
      opts.attributes.push('ismemberof');
    }else{
      opts.attributes = [];
      opts.attributes.push('ismemberof');
    }
    
    self._adminClient.search(self.opts.searchBaseGroups, opts, function (err, result) {
      if (err) {
        self.log && self.log.trace('ldap authenticate: search error: %s %s %s', err.code, err.name, err.message);
        return callback(err);
      }
      var groups = [];
      result.on('searchEntry', function (entry) {
        //console.log(entry.object.ismemberof);
        groups = entry.object.ismemberof;
      });
      result.on('error', function (err) {
        self.log && self.log.trace(
          'ldap authenticate: search error event: %s %s %s', err.code, err.name, err.message);
      ...
      result.on('end', function (result) {
        console.log('group result: ' + inspect(result.messageID));
        if (result.status !== 0) {
          var err = 'non-zero status from LDAP search: ' + result.status;
          self.log && self.log.trace('ldap authenticate: %s', err);
          return callback(err);
        ...
        switch (groups.length) {
          case 0: // NO GROUP FOUND
            return callback();
          default: // GROUP(S) FOUND
...
LdapAuth.prototype.authenticate = function (username, password, callback) {
  ...
  self._findUser(username, function (err, user) {
    ...
    // 2. Find the groups
    self._findGroups(username, function (err, groups){
      if (err)
        return callback(err);
      user.groups = groups;

      // 3. Attempt to bind as that user to check password.
